		...
		If ($CookieTeamNumber > 0 AND $CookieTeamNumber <= 102){
			...
			If ($TeamEdit == $CookieTeamNumber){	
				...
				if (isset($_POST['DraftYear'])) {
					...
					$PlayerDraftYear = ($PlayerDraftYear >= 0 && $PlayerDraftYear <= 3000) ? (int)$PlayerDraftYear : 0;
				} else {
					$PlayerDraftYear = 0;
				}
...
if (empty($PlayerInfo) == false){while ($Row = $PlayerInfo ->fetchArray()) { 
	...
	echo "<td class=\"STHSCenter\"><input type=\"number\" min=\"0\" max=\"3000\" name=\"DraftYear\" value=\"";If(isset($Row['DraftYear'])){Echo $Row['DraftYear'];}echo "\"></td>";
